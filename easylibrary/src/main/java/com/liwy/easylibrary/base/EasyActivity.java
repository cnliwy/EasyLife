...
public abstract class EasyActivity extends AppCompatActivity {
    ...
    private static final int defalutBackIconResID = R2.drawable.ic_menu;          // 左侧返回按钮的默认图片
    // toolbar标题显示位置
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mContext = this;
        EasyApplication.getInstance().addActivity(this);
        // 初始化toolbar
        initToolbar();
        if (toolbar != null){
            //如果页面导入了toolbar则显示
    ...
    protected void onDestroy() {
        super.onDestroy();
        EasyApplication.getInstance().removeActivity(this);
    }
